...
class TypesOfReport(models.Model):
    WEEK = 1
    MONTH = 2
    TERMESTER = 3
    SEMESTER = 4
    ...
    def save(self,*args,**kwargs):
        ...
        super(TypesOfReport,self).save(*args,**kwargs)
...
class Report(models.Model):
    ...
    report_name     = models.ForeignKey(TypesOfReport, on_delete=models.CASCADE)
    report_file     = models.FileField(upload_to='reports')
    submitted_on   = models.DateTimeField(auto_now_add=True,blank=True, null=True)
    time_of_report = models.PositiveSmallIntegerField(choices=TIME_CHOICE)
