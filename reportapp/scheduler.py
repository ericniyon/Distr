...
def all_users_emails_week():
    weekly = []
    for email in TypesOfReport.objects.all():
...
def start(deadline):
    ...
    reports_n = TypesOfReport.objects.all()
    reports = TypesOfReport.objects.all()



    for simple in reports_n:
        if simple.igihe_itangirwa == 1:
            ...
            # remainderWeek = datetime.datetime.now()+ timedelta(seconds=2)
